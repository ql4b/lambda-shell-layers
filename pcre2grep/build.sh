...
# Test the binary
echo "Testing binary..."
docker build --platform $PLATFORM --target pcre2grep-installer -t lambda-shell-$LAYER_NAME-test .
docker run --platform $PLATFORM --entrypoint="/bin/sh" --rm lambda-shell-$LAYER_NAME-test -c "echo -e 'body { color: #ff3a27; background: #222; }' | pcre2grep '#[0-9a-fA-F]{3,6}' && echo '✓ pcre2grep test passed'"
