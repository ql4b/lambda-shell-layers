...
# Test the binary
echo "Testing binary..."
docker build --platform $PLATFORM --target htmlq-builder -t lambda-shell-$LAYER_NAME-test .
docker run --platform $PLATFORM --entrypoint /bin/sh --rm lambda-shell-$LAYER_NAME-test -c "echo '<html><body><h1>Test</h1></body></html>' | /opt/bin/htmlq 'h1' --text && echo '✓ htmlq test passed'"
